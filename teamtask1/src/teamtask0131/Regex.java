...
public class Regex {
	...
	boolean regexName(String name) {
		String namePattern = "^(?=.[가-힣])[가-힣]{2,5}$";
	...
	boolean regexPosLibraryName(String name, String writer,String posLibrary) {
		String posLibraryPattern = "^[a-zA-Z]{3,7}$";
		...
		if (!(matcher.matches())) {
			System.out.println("알맞은 구비처를 입력해주세요. (libaaa, libbbb)");
			return false;
		}
		...
		if (!(posLibrary.contains("libaaa") || posLibrary.contains("libbbb"))) {
			System.out.println("libaaa 도서관 또는 libbbb 도서관을 입력해주세요.");
			return false;
		}
		return true;
	}
